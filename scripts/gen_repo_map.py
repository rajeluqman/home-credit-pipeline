...
Ported from creative_intelligence_lab's scripts/gen_repo_map.py (pipeline-retrofit effort),
retargeted to this repo's real layout (glue/, bronze/, silver/, dbt_home_credit/, airflow/dags/,
gx/, docs/, docs/ADR/) instead of CIL's models/seeds/dags-at-root layout.
...
EXCLUDE = {
    ".gitignore", ".env.example", "dbt_home_credit/profiles.yml",
...
ROLE_LABELS = [
    ...
    ("dbt:model", "dbt — other"),
    ("glue", "AWS Glue (Silver, PySpark)"),
    ("bronze", "Bronze ingestion"),
    ("silver", "Silver (local/pandas mirror)"),
    ("dag", "Airflow DAGs"),
...
def role_of(rel: str) -> str:
    ...
    if rel.startswith("docs/"):
        return "doc"
    if rel.startswith("dbt_home_credit/models/staging/"):
        return "dbt:staging"
    if rel.startswith("dbt_home_credit/models/intermediate/"):
        return "dbt:intermediate"
    if rel.startswith("dbt_home_credit/models/mart/"):
        return "dbt:mart"
    if rel.startswith("dbt_home_credit/snapshots/"):
        return "dbt:snapshot"
    if rel.startswith("dbt_home_credit/tests/"):
        return "dbt:test"
    if rel.startswith("dbt_home_credit/macros/"):
        return "dbt:macro"
    if rel.startswith("dbt_home_credit/"):
        return "dbt:model"
