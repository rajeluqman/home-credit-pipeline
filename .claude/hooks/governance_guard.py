...
files. Ported from creative_intelligence_lab's hook of the same name (pipeline-retrofit effort);
retargeted to this repo's real files (Glue/dbt/Snowflake, not DuckDB/dbt-duckdb).
...
Two contracts, two owners (CLAUDE.md governance axes):
  ...
  - tests/boundary_contract.py  — @scope-guardian (Spark only inside Glue, Databricks
                                   query-only, no new ingestion connectors)
...
BOUNDARY_MSG = (
    "no standalone PySpark outside glue/, no Databricks Classic Compute/job clusters "
    "(Serverless SQL query-only), no new ingestion connector beyond the Kaggle API."
)
...
RULES: list[tuple[str, str, str, tuple[str, ...]]] = [
    ("glue/glue_silver_", "ADR-002 (PII mask order) + docs/ARCHITECTURE.md", BOUNDARY_MSG, ("boundary_contract.py",)),
    ("dbt_home_credit/models/mart/", "ADR-001 + docs/DATA_MODEL.md (.claude/agents/data-architect.md)", ERD_MSG, ("identity_contract.py",)),
    ("dbt_home_credit/snapshots/", "ADR-001 SCD2 strategy + docs/DATA_MODEL.md", IDENTITY_MSG, ("identity_contract.py",)),
    ("dbt_home_credit/profiles.yml", "docs/ARCHITECTURE.md — Snowflake is the sole Gold compute target", BOUNDARY_MSG, ("boundary_contract.py",)),
    ("dbt_home_credit/dbt_project.yml", "docs/ARCHITECTURE.md stack boundary", BOUNDARY_MSG, ("boundary_contract.py",)),
    ("airflow/dags/", "docs/ARCHITECTURE.md + docs/PIPELINE_SPEC.md (DAG chaining)", BOUNDARY_MSG, ("boundary_contract.py",)),
    ("requirements.txt", "docs/ARCHITECTURE.md stack boundary (no Spark outside Glue, no Databricks Classic Compute)", BOUNDARY_MSG, ("boundary_contract.py",)),
