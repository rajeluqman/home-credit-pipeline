...
# Airflow DAG
# =============================================================================
# NOTE (2026-06-29 audit, ahead of a Microsoft Fabric migration): this script used to
# scaffold a single monolithic airflow/dags/pipeline_dag.py here, referencing
# bronze.bronze_pipeline / silver.trigger_glue_job — modules that were never built (this
# repo's real Bronze/Silver code is bronze/ingest_bronze.py + glue/glue_silver_*.py). That
# scaffold was superseded by the real, separately-built bronze_ingestion_dag.py /
# silver_transforms_dag.py / gold_dbt_dag.py (see README.md "Project Structure"), but the
# orphaned generated file was never deleted from the repo and would silently fail at runtime
# if Airflow ever executed it. Removed here + the committed file deleted — see DECISION_LOG.md.

echo "✅ Airflow DAGs already created (bronze_ingestion_dag.py / silver_transforms_dag.py / gold_dbt_dag.py)"
