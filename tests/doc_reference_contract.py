...
effort) — same mechanism, retargeted to this repo's dbt project root (`dbt_home_credit/`
not `models/`/`seeds/` at repo root). Proves every model/path a Markdown doc references
ACTUALLY EXISTS, so doc drift fails the build instead of misleading the next reader.
...
What it checks:
  C1  MODEL refs — backtick-wrapped tokens shaped like a dbt object (prefix
      ...
      dbt_home_credit/models/**/*.sql or a real snapshot under dbt_home_credit/snapshots/*.sql.
  C2  PATH refs — backtick tokens and []() link targets that point at a repo path
      (glue/ dbt_home_credit/ docs/ tests/ airflow/ .claude/ gx/ scripts/) must exist on disk.
...
PATH_ROOTS = ("glue/", "dbt_home_credit/", "docs/", "tests/", "airflow/", ".claude/",
              "gx/", "scripts/", "bronze/", "silver/", "architecture/")
            ...
            for tok in backtick.findall(line):
                ...
                if MODEL_TOKEN.match(tok) and tok not in known and tok not in ALLOW:
                    errors.append(
                        ...
                        f"dbt_home_credit/models|snapshots/**/{tok}.sql exists (drift)"
                    )
