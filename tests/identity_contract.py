...
Ported from creative_intelligence_lab's tests/lineage_contract.py pattern (rules as code,
not vigilance) — renamed "identity contract" per the Opus retrofit ruling, since this repo's
identity key is SK_ID_CURR (`applicant_id`), not a content-hash asset_id. Encodes
@data-architect's "exactly 1 is_current=TRUE row per applicant" SCD2 guarantee statically,
WITHOUT requiring a live Snowflake connection — it checks the dbt SOURCE (snapshot config +
mart alias SQL), not warehouse data. The dbt-native test
`dbt_home_credit/tests/assert_scd2_one_current_per_applicant.sql` is the runtime complement
(needs `dbt test` against real data); this script is the $0 static gate that runs in CI
without credentials.
